...
# 检查 node-pty 架构是否为 x64
PTY_NODE="node_modules/node-pty/build/Release/pty.node"
if [ -f "$PTY_NODE" ]; then
  PTY_ARCH=$(file "$PTY_NODE" | grep -o 'arm64\|x86_64' | head -1)
  if [ "$PTY_ARCH" != "x86_64" ]; then
    echo "⚠️  node-pty 架构 ($PTY_ARCH) 不是 x64"
    echo "🔄 重新编译 node-pty 为 x64..."
    npx @electron/rebuild -f -w node-pty -a x64
    echo "✅ node-pty 重新编译完成"
    echo ""
  fi
else
  echo "⚠️  node-pty 未编译，正在编译为 x64..."
  npx @electron/rebuild -f -w node-pty -a x64
  echo "✅ node-pty 编译完成"
  echo ""
fi
...
APP_PATH="$DIST_DIR/mac/GitManager.app"
if [ -d "$APP_PATH" ]; then
  echo "📝 移除隔离属性..."
  xattr -cr "$APP_PATH" 2>/dev/null || true
  
  echo "📝 正在签名: GitManager.app (X64)"
...
echo ""
echo "📦 步骤4: 创建分发包..."
rm -f "$DIST_DIR"/GitManager-*-mac.zip "$DIST_DIR"/GitManager-*.dmg "$DIST_DIR"/*.blockmap 2>/dev/null || true
...
echo ""
echo "📦 已生成以下文件："
ls -lh "$DIST_DIR"/GitManager-*.dmg "$DIST_DIR"/GitManager-*-mac.zip 2>/dev/null | grep -v arm64 || true
echo ""
