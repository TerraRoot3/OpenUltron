...
const definition = {
  ...
  parameters: {
    ...
    properties: {
      ...
      model: { type: 'string', description: '可选。子 Agent 使用的模型 ID，不传则使用该供应商的默认模型；需与 provider 对应供应商的可用模型一致（见 list_providers_and_models）' },
...
function createSessionsSpawnTool(runSubChat) {
  ...
  async function execute(args, context = {}) {
    const { task, system_prompt, provider, model, project_path, role_name } = args || {}
