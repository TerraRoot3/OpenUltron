...
export function installBrowserPolyfill(baseUrl = DEFAULT_API_BASE) {
  ...
  window.electronAPI = {
    ...
    ai: {
      ...
      listRemoteWebApps: () => invoke('web-apps-list-remote', []),
      installRemoteWebApps: (data) => invoke('web-apps-install-remote', [data || {}]),
      checkRemoteWebAppUpdates: (data) => invoke('web-apps-check-updates', [data || {}]),
      updateRemoteWebApps: (data) => invoke('web-apps-update-remote', [data || {}]),
      publishWebApp: (data) => invoke('web-apps-publish', [data || {}]),
